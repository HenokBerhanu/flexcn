...
std::vector<AMFData> Cassandra5GDB::get_amf_data_from_cass_result(
...
bool Cassandra5GDB::insert_5g_context(
    const AMFData& data, const std::string& ids, int id) {
...
std::vector<AMFData> Cassandra5GDB::get_row_by_id(int id) {
  ...
  Logger::flexcn_app().info(
      "[5g amf] not support the feature [get row by internal id]");
  return std::vector<AMFData>{};
}

std::vector<AMFData> Cassandra5GDB::get_row_by_imsi_bearer_id(
