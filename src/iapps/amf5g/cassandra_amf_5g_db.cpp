...
std::vector<AMFData> Cassandra5GDB::select_query(std::string str_query){
    ...
    if(cass_future_error_code(result_future) == CASS_OK) {
      ...
      v_b = get_amf_data_from_cass_result(result);
...
AMFData make_amf_data_from_text(const std::string &data){
    return AMFData(data);
}

AMFData Cassandra5GDB::get_a_amf_data_from_a_cass_row(const CassRow* row){
    
    std::string data = get_string_from_cass_value(row, "data");
    return make_amf_data_from_text(data);
}

std::vector<AMFData> Cassandra5GDB::get_amf_data_from_cass_result(const CassResult* result) {
    
    std::vector<AMFData> v_bearers;
    CassIterator* rows = cass_iterator_from_result(result);

      while(cass_iterator_next(rows)) {
        const CassRow* row = cass_iterator_get_row(rows);
        AMFData br = get_a_amf_data_from_a_cass_row(row);
...
std::string to_text(const AMFData &data){
    return std::string(data);
}

bool Cassandra5GDB::insert_5g_context(const AMFData& data, const std::string& ids, int id){
...
std::vector<AMFData> Cassandra5GDB::get_all_rows(){
    return select_query("SELECT * FROM " + m_space + "."  + m_table_5g_context + ";");
}

std::vector<AMFData> Cassandra5GDB::get_row_by_id(int id){    
    // not support yet
    Logger::flexcn_app().info("[5g amf] not support the feature [get row by internal id]");
    return std::vector<AMFData>{};
}

std::vector<AMFData> Cassandra5GDB::get_row_by_imsi_bearer_id(std::string supi, int bearerID){
