...
AMFApp::AMFApp(
    const std::string& flexcn_ip, int flexcn_port, const std::string& amf_ip,
    int amf_port)
    : MonitoringAppAPI() {
...
//------------------------------------------------------------------------------
bool trigger_amf_notification_subscribe(
    const std::string& addr4, int port, const std::string& amf_addr_ipv4,
    int amf_addr_port) {
  ...
  json_data["eventNotifyUri"] =
      addr4 + ":" + std::to_string(port) + "/flexcn-status-notify/v1/amf001";

  json_data["notifyCorrelationId"] =
      addr4 + ":" + std::to_string(port) + "/flexcn-status-notify/v1/amf001";
  ...
  std::string url = amf_addr_ipv4 + ":" + std::to_string(amf_addr_port) +
...
bool AMFApp::persist_data(
  ...
  if (!json_data.contains(EVENT_DATA_KEY)) {
    Logger::flexcn_app().error(
        "Message don't contains %s key!", EVENT_DATA_KEY);
    Logger::flexcn_app().error(data_event.c_str());
    return false;
  }
  ...
  for (auto j : json_data[EVENT_DATA_KEY]) {
    ...
    try {
      ...
      // can't trigger this event from CN
        ...
        Logger::flexcn_app().info(
            "Cannot recongize this event type: %s", type.c_str());
      }
...
std::string AMFApp::retrieve_data_by_key(const std::string& supi) {
  ...
  Logger::flexcn_app().error(
      "AMF iAPP does not have the data that is related to key [ %s ]",
      supi.c_str());
  return "";
}
