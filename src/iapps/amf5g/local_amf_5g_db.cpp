...
Local5GAMFDB::Local5GAMFDB(const std::string& database_ip)
    : Database5GAMFWrapper(database_ip), m_main_database_5g() {}

Local5GAMFDB::~Local5GAMFDB() {
  std::cout << "Clean alll" << std::endl;
}

bool Local5GAMFDB::insert_5g_context(
...
bool Local5GAMFDB::delete_by_internal_id(int id) {
  Logger::flexcn_app().info(
      "Function delete_by_internal_id() of AMF App is NOT "
...
std::vector<AMFData> Local5GAMFDB::get_row_by_id(int id) {
  std::vector<AMFData> stats;
  Logger::flexcn_app().info(
      "Function get_row_by_id() of AMF App is NOT supported/implemented");
  return stats;
}

std::vector<AMFData> Local5GAMFDB::get_row_by_key(
