...
Cassandra5GDB::Cassandra5GDB(const std::string& database_ip) : Database5GWrapper(database_ip) {
...
void Cassandra5GDB::setup_insert_query(){
    std::string str_insert_5g_query = "INSERT INTO " + m_space + "."  + m_table_5g_context + " (row_id, extra_id, supi, data) VALUES (?, ?, ?, ?);";
    // CassString insert_query = cass_string_init(str_insert_5g_query.c_str());

    /* Prepare the statement on the Cassandra cluster */
    CassFuture* prepare_future = cass_session_prepare(m_session, str_insert_5g_query.c_str());
...
//     std::string query =  "SELECT row_id FROM "  + m_space + "." + m_table_5g_context + 
...
std::vector<SMFData> Cassandra5GDB::get_smf_data_from_cass_result(const CassResult* result) {
...
void Cassandra5GDB::create_space_and_table(std::string space_name, std::string table_name){
  ...
  rc = query("CREATE TABLE " + space_name + "."  + table_name + " (row_id int, extra_id int, supi text PRIMARY KEY, data text);");
...
//   rc = query("DROP TABLE IF EXISTS " + space_name + "."  + table_name + "_by_supi");
//   if (rc != CASS_OK) {
//       std::cout << "Failed to drop table if exists" << std::endl;
//   }
//   else {
...
//   rc = query("CREATE TABLE " + space_name + "."  + table_name + "_by_supi (row_id int, supi text, PRIMARY KEY(supi));");
...
bool Cassandra5GDB::insert_5g_context(const SMFData& data, const std::string& ids, int id){
    ...
    cass_statement_bind_string_by_name(m_insert_5g_statement, "supi", ids.c_str());
    cass_statement_bind_string_by_name(m_insert_5g_statement, "data", to_text(data).c_str());
    

    // std::cout << "INSERT INTO " + m_space + "."  + m_table_5g_context + " (row_id, supi) VALUES (" 
    //                     + std::to_string(id) + "," + std::to_string(bearer.getEpsSMFDataId()) + "," + bearer.getImsi() +" );" << std::endl;
    // CassError rc = query("INSERT INTO " + m_space + "."  + m_table_5g_context + "_by_row_id (row_id, eps_bearer_id,imsi) VALUES (" 
    //                     + std::to_string(id) + "," + std::to_string(bearer.getEpsSMFDataId()) + ",\'" + bearer.getImsi() +"\' );");
    ...
    // cass_statement_bind_string_by_name(m_insert_5g_statement, "s1_ul_teid", bearer.getS1UlTeid().c_str());
    
    // /// S1 uplink tunnel ID   
    // cass_statement_bind_string_by_name(m_insert_5g_statement, "s1_dl_teid", bearer.getS1DlTeid().c_str());


    // /// IP address of UE
    // cass_statement_bind_string_by_name(m_insert_5g_statement, "ue_ip", bearer.getUeIp().c_str());

    
    // /// IP address of eNodeB
    // cass_statement_bind_string_by_name(m_insert_5g_statement, "enb_ip", bearer.getEnbIp().c_str());

        /* Execute statement (same a the non-prepared code) */
        CassFuture* query_future = cass_session_execute(m_session, m_insert_5g_statement);
...
//     rc = query(" TRUNCATE " + m_space + "."  + m_table_5g_context + "_by_row_id ;");
//     if (rc != CASS_OK) {
...
std::vector<SMFData> Cassandra5GDB::get_row_by_id(int id){    
    // not support yet
    Logger::flexcn_app().info("[5g smf] not support the feature [get row by internal id]");
    return std::vector<SMFData>{};
    // return select_query("SELECT * FROM " + m_space + "."  + m_table_5g_context + " WHERE supi = " + std::to_string(id) + ";");
}

std::vector<SMFData> Cassandra5GDB::get_row_by_imsi_bearer_id(std::string supi, int bearerID){
    return select_query("SELECT * FROM " + m_space + "."  + m_table_5g_context + " WHERE supi = \'" + supi + "\' ALLOW FILTERING;");
}
