...
Local5GDB::Local5GDB(const std::string& database_ip) : Database5GWrapper(database_ip), m_main_database_5g(){
}
...
bool Local5GDB::insert_5g_context(const SMFData& data, const std::string& ids, int id){
  m_main_database_5g[ids][id] = std::move(data);  
  return true;
}

bool Local5GDB::delete_by_internal_id(int id) {
  Logger::flexcn_app().info("Function delete_by_internal_id() of SMF App is NOT supported/implemented");
  return false;
}
...
std::vector<SMFData> Local5GDB::get_row_by_id(int id){
   std::vector<SMFData> stats;
     Logger::flexcn_app().info("Function get_row_by_id() of SMF App is NOT supported/implemented");
  return stats;
}
