...
public:
  Database5GWrapper(const std::string& database_ip){};
  ~Database5GWrapper(){};

  virtual bool insert_5g_context(const SMFData& bearer,const std::string& ids, int id) = 0;
  ...
  virtual std::vector<SMFData> get_all_rows() = 0;
  virtual std::vector<SMFData> get_row_by_id(int id) = 0;
  virtual std::vector<SMFData> get_row_by_key(std::string key, int extraID) = 0;
