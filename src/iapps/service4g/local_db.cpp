...
Local4GDB::Local4GDB(const std::string& database_ip)
    : DatabaseWrapper(database_ip),
      m_main_database_4g(),
      m_imsi_bearer_id_to_main_id() {}
...
bool Local4GDB::insert_4g_context(const Bearer& bearer, int id) {
  ...
  m_main_database_4g.insert(
      std::pair<unsigned int, Bearer>(id, std::move(bearer)));
  m_imsi_bearer_id_to_main_id[imsi].insert(
      std::pair<unsigned int, unsigned int>(bearer_id, id));
...
bool Local4GDB::delete_by_internal_id(int id) {
  try {
    ...
    unsigned int bearer_id =
        (unsigned int) m_main_database_4g[id].getEpsBearerId();
...
bool Local4GDB::delete_by_imsi_bearer_id(std::string imsi, int bearer_id) {
  try {
    if (m_imsi_bearer_id_to_main_id.find(imsi) ==
        m_imsi_bearer_id_to_main_id.end()) {
