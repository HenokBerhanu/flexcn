...
Cassandra4GDB::Cassandra4GDB(const std::string& database_ip) : DatabaseWrapper(database_ip) {
...
void Cassandra4GDB::setup_insert_query(){
    std::string str_insert_4g_query = "INSERT INTO " + m_space + "."  + m_table_4g_context + " (row_id, eps_bearer_id,eps_meter_id,imsi,s1_ul_teid, s1_dl_teid, ue_ip,enb_ip) VALUES (?, ?, ?, ?, ?, ?, ?, ?);"; // row_id
    // CassString insert_query = cass_string_init(str_insert_4g_query.c_str());

    /* Prepare the statement on the Cassandra cluster */
    CassFuture* prepare_future = cass_session_prepare(m_session, str_insert_4g_query.c_str());
...
int Cassandra4GDB::query_index(std::string imsi, int bearerID) {
    std::string query =  "SELECT row_id FROM "  + m_space + "." + m_table_4g_context + 
                "_by_row_id WHERE imsi=\'" + imsi + "\' and eps_bearer_id= " + std::to_string(bearerID) + ";";
...
std::string get_string_from_cass_value(const CassRow *row, std::string name_col){
...
std::vector<Bearer> Cassandra4GDB::get_bearers_from_cass_result(const CassResult* result) {
...
void Cassandra4GDB::create_space_and_table(std::string space_name, std::string table_name){

    // setting up the new keyspace for data
  CassError rc = query("CREATE KEYSPACE IF NOT EXISTS " + space_name + " WITH REPLICATION = { 'class' : 'SimpleStrategy', 'replication_factor' : '1' };");
  ...
  rc = query("CREATE TABLE " + space_name + "."  + table_name + " (row_id int PRIMARY KEY, eps_bearer_id int, eps_meter_id int, imsi text, s1_ul_teid text, s1_dl_teid text, ue_ip text, enb_ip text);"); // , row_id int
  ...
  rc = query("DROP TABLE IF EXISTS " + space_name + "."  + table_name + "_by_row_id");
  if (rc != CASS_OK) {
      std::cout << "Failed to drop table if exists" << std::endl;
  }
  else {
  ...
  rc = query("CREATE TABLE " + space_name + "."  + table_name + "_by_row_id (row_id int, eps_bearer_id int, imsi text, PRIMARY KEY(imsi, eps_bearer_id));"); // , row_id int
...
bool Cassandra4GDB::insert_4g_context(const Bearer& bearer, int id){
    ...
    cass_statement_bind_int32_by_name(m_insert_4g_statement, "eps_bearer_id", bearer.getEpsBearerId());
    cass_statement_bind_int32_by_name(m_insert_4g_statement, "eps_meter_id", bearer.getEpsMeterId());

    /// (optional) Type of service including DSCP and ECN.
    // int64_t getTos() const;
    
    cass_statement_bind_string_by_name(m_insert_4g_statement, "imsi", bearer.getImsi().c_str());

    std::cout << "INSERT INTO " + m_space + "."  + m_table_4g_context + "_by_row_id (row_id, eps_bearer_id,imsi) VALUES (" 
                        + std::to_string(id) + "," + std::to_string(bearer.getEpsBearerId()) + "," + bearer.getImsi() +" );" << std::endl;
    CassError rc = query("INSERT INTO " + m_space + "."  + m_table_4g_context + "_by_row_id (row_id, eps_bearer_id,imsi) VALUES (" 
                        + std::to_string(id) + "," + std::to_string(bearer.getEpsBearerId()) + ",\'" + bearer.getImsi() +"\' );");
    ...
    cass_statement_bind_string_by_name(m_insert_4g_statement, "s1_ul_teid", bearer.getS1UlTeid().c_str());
    
    /// S1 uplink tunnel ID   
    cass_statement_bind_string_by_name(m_insert_4g_statement, "s1_dl_teid", bearer.getS1DlTeid().c_str());


    /// IP address of UE
    cass_statement_bind_string_by_name(m_insert_4g_statement, "ue_ip", bearer.getUeIp().c_str());

    
    /// IP address of eNodeB
    cass_statement_bind_string_by_name(m_insert_4g_statement, "enb_ip", bearer.getEnbIp().c_str());

        /* Execute statement (same a the non-prepared code) */
        CassFuture* query_future = cass_session_execute(m_session, m_insert_4g_statement);
...
bool Cassandra4GDB::delete_by_internal_id(int id) {
  ...
  if (brs.size()> 0){
      ...
      CassError rc = query("DELETE FROM " + m_space + "."  + 
            m_table_4g_context + "_by_row_id WHERE imsi = \'" +
  ...
  CassError rc = query("DELETE FROM " + m_space + "."  + m_table_4g_context +
...
bool Cassandra4GDB::delete_by_imsi_bearer_id(std::string imsi, int bearerID) {
  ...
  if (index < 0){
      std::cout << "Don't find imsi " << imsi << " bearer " << bearerID << std::endl;
      return false;
  }
    std::string del_query =  "DELETE FROM " + m_space + "."  + 
            m_table_4g_context + "_by_row_id WHERE imsi = \'" +
            imsi + "\' and eps_bearer_id = " + 
            std::to_string(bearerID) + " IF EXISTS;";
  ...
  CassError rc = query(del_query);
   ...
   rc = query("DELETE FROM " + m_space + "."  + m_table_4g_context +
...
bool Cassandra4GDB::delete_all() {
    ...
    rc = query(" TRUNCATE " + m_space + "."  + m_table_4g_context + "_by_row_id ;");
    if (rc != CASS_OK) {
...
std::vector<Bearer> Cassandra4GDB::get_all_rows(){
    return select_query("SELECT * FROM " + m_space + "."  + m_table_4g_context + ";");
}

std::vector<Bearer> Cassandra4GDB::get_row_by_id(int id){
    return select_query("SELECT * FROM " + m_space + "."  + m_table_4g_context + " WHERE row_id = " + std::to_string(id) + ";");
}

std::vector<Bearer> Cassandra4GDB::get_row_by_imsi_bearer_id(std::string imsi, int bearerID){
    return select_query("SELECT * FROM " + m_space + "."  + m_table_4g_context + " WHERE imsi = \'" + imsi + "\' AND eps_bearer_id = " + std::to_string(bearerID) + " ALLOW FILTERING;");
}
