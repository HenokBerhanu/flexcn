...
#ifndef FILE_CASSANDRA_4G_DB_HPP_SEEN
#define FILE_CASSANDRA_4G_DB_HPP_SEEN
...
class Cassandra4GDB : public DatabaseWrapper {
...
private:
  ...
  std::vector<Bearer> select_query(std::string str_query);
  Bearer get_a_bearer_from_a_cass_row(const CassRow* row);
  std::vector<Bearer> get_bearers_from_cass_result(const CassResult* result);
