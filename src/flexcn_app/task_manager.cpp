...
//------------------------------------------------------------------------------
task_manager::task_manager(flexcn_event& ev) : event_sub_(ev) {
  ...
  if (timerfd_settime(sfd, TFD_TIMER_ABSTIME, &its, NULL) == -1) {
    Logger::flexcn_app().error("Failed to set timer for task manager");
  }
}
...
void task_manager::wait_for_cycle() {
  ...
  if (sfd > 0) {
    ...
    if ((res < 0) || (res != sizeof(exp))) {
      Logger::flexcn_app().error("Failed in task manager timer wait");
    }
  }
}
