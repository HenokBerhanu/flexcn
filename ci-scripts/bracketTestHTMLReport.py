...
class HtmlReport():
	...
	def generate(self):
		cwd = os.getcwd()
		self.file = open(cwd + '/test_results_oai_flexcn_bt.html', 'w')
		self.generateHeader()
	...
	def deploymentSummaryHeader(self):
		...
		self.file.write('     </tr>\n')
		self.addImageRow('oai_flexcn')
		self.file.write('  </table>\n')
		self.file.write('  </div>\n')

	def addImageRow(self, imageInfoPrefix):
		cwd = os.getcwd()
		if imageInfoPrefix == 'oai_flexcn':
			containerName = 'oai-flexcn'
			tagPattern = 'OAI_FLEXCN_TAG'
			statusPrefix = 'flexcn'
	...
	def testSummaryHeader(self):
		...
		cwd = os.getcwd()
		finalStatusOK = False
		if os.path.isfile(cwd + '/DS-TEST-RESULTS/dcflexcn2.yaml'):
			cmd = f'egrep -c "final-result: pass" DS-TEST-RESULTS/dcflexcn2.yaml || true'
			ret = subprocess.run(cmd, shell=True, stdout=subprocess.PIPE, encoding='utf-8')
			if ret.stdout is not None:
				if ret.stdout.strip() == '1':
					finalStatusOK = True
	...
	def testSummaryDetails(self):
		...
		cwd = os.getcwd()
		if os.path.isfile(cwd + '/DS-TEST-RESULTS/dcflexcn2.yaml'):
			with open(cwd + '/DS-TEST-RESULTS/dcflexcn2.yaml') as f:
				data = yaml.load(f)
...
def Usage():
	...
	print('   Generate an HTML report for the Jenkins pipeline on oai-cn5g-flexcn-bt.')
	print('----------------------------------------------------------------------------------------------------------------------')
