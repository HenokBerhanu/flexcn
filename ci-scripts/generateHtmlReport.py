...
class HtmlReport():
	...
	def generate(self):
		cwd = os.getcwd()
		self.file = open(cwd + '/test_results_oai_flexcn.html', 'w')
		self.generateHeader()
	...
	def generateHeader(self):
		...
		for reportFile in glob.glob('./*results_oai_*.html'):
			if reportFile == './test_results_oai_flexcn.html':
				continue
	...
	def buildSummaryHeader(self):
		...
		self.file.write('		<th>OAI FLEXCN <font color="Gold">Ubuntu18</font> Image</th>\n')
		self.file.write('		<th>OAI FLEXCN <font color="Gold">RHEL8</font> Image</th>\n')
		self.file.write('     </tr>\n')
	...
	def initialGitSetup(self):
		...
		self.analyze_docker_build_git_part('FLEXCN')
		self.file.write('	 </tr>\n')
	...
	def analyze_docker_build_git_part(self, nfType):
		...
		for variant in variants:
			logFileName = 'flexcn_' + variant + '_image_build.log'
			if os.path.isfile(cwd + '/archives/' + logFileName):
				status = False
				section_start_pattern = 'git config --global http'
				section_end_pattern = 'WORKDIR /openair-flexcn/build/scripts'
	...
	def installLibsPackagesRow(self):
		...
		self.analyze_install_log('FLEXCN')
		self.file.write('	 </tr>\n')
	...
	def analyze_install_log(self, nfType):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				status = False
				section_start_pattern = 'build_flexcn --install-deps --force'
				section_end_pattern = 'build_flexcn --clean --Verbose --build-type Release --jobs'
				section_status = False
				...
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
						result = re.search('FROM oai-flexcn-base', line)
						if result is not None:
							base_image = True
						result = re.search(section_start_pattern, line)
						if result is not None:
						...
						if result is not None:
							section_status = False
						if section_status:
							result = re.search('FLEXCN deps installation successful', line)
							if result is not None:
								status = True
				...
				if base_image:
					...
					cell_msg = '	  <td bgcolor="Tomato"><pre style="border:none; background-color:Tomato"><b>'
					cell_msg += 'KO:\n'
				cell_msg += ' -- build_flexcn --install-deps --force\n'
				if base_image:
	...
	def buildCompileRows(self):
		...
		self.analyze_build_log('FLEXCN', True)
		self.file.write('	 </tr>\n')
		self.file.write('	 <tr>\n')
		self.analyze_compile_log('FLEXCN', True)
		self.file.write('	 </tr>\n')
	...
	def analyze_build_log(self, nfType, imageKind):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				status = False
				if nfType == 'FLEXCN':
					section_start_pattern = 'build_flexcn --clean --Verbose --build-type Release --jobs'
					section_end_pattern = 'FROM ubuntu:bionic as oai-flexcn$'
					pass_pattern = 'flexcn installed'
				section_status = False
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
						result = re.search(section_start_pattern, line)
				...
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
						...
						if section_status:
							...
							if result is not None:
								correctLine = re.sub("^.*/openair-flexcn","/openair-flexcn",line.strip())
	...
	def analyze_copy_log(self, nfType):
		...
		for variant in variants:
			logFileName = 'flexcn_' + variant + '_image_build.log'
			if os.path.isfile(cwd + '/archives/' + logFileName):
				section_start_pattern = 'FROM ubuntu:bionic as oai-flexcn$'
				section_end_pattern = 'COPY --from=oai-flexcn-builder /openair-flexcn/scripts/entrypoint.sh entrypoint.sh'
				section_status = False
				status = False
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
	...
	def copyConfToolsToTargetImage(self):
		...
		self.analyze_copy_conf_tool_log('FLEXCN')
		self.file.write('	 </tr>\n')
	...
	def analyze_copy_conf_tool_log(self, nfType):
		...
		for variant in variants:
			logFileName = 'flexcn_' + variant + '_image_build.log'
			if os.path.isfile(cwd + '/archives/' + logFileName):
				section_start_pattern = 'WORKDIR /openair-flexcn/etc'
				if variant == 'docker':
					section_end_pattern = 'Successfully tagged oai-flexcn'
				else:
					section_end_pattern = 'COMMIT oai-flexcn:'
				section_status = False
				status = False
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
	...
	def imageSizeRow(self):
		...
		self.analyze_image_size_log('FLEXCN', True)
		self.file.write('	 </tr>\n')
	...
	def analyze_image_size_log(self, nfType, imageKind):
		...
		for variant in variants:
			...
			if os.path.isfile(cwd + '/archives/' + logFileName):
				if nfType == 'FLEXCN':
					if variant == 'docker':
						section_start_pattern = 'Successfully tagged oai-flexcn'
						section_end_pattern = 'OAI-FLEXCN DOCKER IMAGE BUILD'
					else:
						section_start_pattern = 'COMMIT oai-flexcn:'
						section_end_pattern = 'OAI-FLEXCN PODMAN RHEL8 IMAGE BUILD'
				section_status = False
				status = False
				with open(cwd + '/archives/' + logFileName, 'r') as logfile:
					for line in logfile:
						...
						if section_status:
							if nfType == 'FLEXCN':
								if self.git_pull_request:
									result = re.search('oai-flexcn *ci-tmp', line)
								else:
									result = re.search('oai-flexcn *develop', line)
							if result is not None:
...
def Usage():
	...
	print('   Generate an HTML report for the Jenkins pipeline on openair-flexcn.')
	print('----------------------------------------------------------------------------------------------------------------------')
